...
class CommunitiesImport implements ToCollection, WithHeadingRow
{
    ...
    public function collection(Collection $rows)
    {
        ...
        try {
            $groupedByBuilder = $rows->groupBy('builder_id');

            foreach ($groupedByBuilder as $builderId => $builderRows) {
                $existingCommunities = Community::where('builder_id', $builderId)->get()->keyBy('name');
                ...
                foreach ($builderRows as $row) {
                    ...
                    $community = $existingCommunities[$row['name']] ?? new Community();
                    if (!$community->exists) {
                    ...
                    $imageNames = isset($row['files']) ? array_filter(array_map('trim', explode(',', preg_replace('/,\s+/', ',', $row['files'])))) : [];
                    $mainImageName = trim($row['main_image'] ?? '');
                    $bannerImageName = trim($row['banner'] ?? '');
                    $allImages = array_filter(array_merge([$mainImageName, $bannerImageName], $imageNames), fn($img) => !empty($img) && strtolower($img) !== 'null');
