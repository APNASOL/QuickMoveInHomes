    ...
    public function uploadProperties(Request $request)
{
    ...
    Cache::put("property_upload_progress_$jobId", 1);

    try {
        // Step 1: Handle ZIP
        Cache::put("property_upload_progress_$jobId", 10);
        ...
        if (!file_exists($zipFilePath)) {
            throw new \Exception("ZIP file missing");
        }
        ...
        Cache::put("property_upload_progress_$jobId", 90); // Finalizing
        sleep(1);
        Cache::put("property_upload_progress_$jobId", 100);
    ...
    public function getPropertyDetails($id)
    {
        ...
        $property = Property::with(['feature', 'hoa'])->findOrFail($id);

        $floorplan = FloorPlan::where('property_id', $property->property_id)->get();
        ...
        $propertyData = [
            ...
            'half_bath' => $property->price_from,
            'size_from' => $property->price_from,
            'size_to' => $property->price_from,
            'construction_status' => $property->price_from,
            'listing_status' => $property->price_from,
            'average_price_per_square' => $property->price_from,
    ...
    public function property_photo_delete($id, $property_id)
    {
        ...
        if ($property->images) {
            $merged_id = array_values(array_diff(json_decode($property->images), [$id]));
        }
